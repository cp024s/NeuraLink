...
class OpType(Enum):
    ...
    # Compute ops
    CONV2D = auto()
    DEPTHWISE_CONV2D = auto()  # Depthwise separable convolution
    MATMUL = auto()
    GEMM = auto()
    
    # Attention ops (Transformer)
    ATTENTION = auto()           # Multi-Head Attention
    SCALED_DOT_PRODUCT = auto()  # Q @ K.T / sqrt(d) @ V
    ...
    # Activation ops
    RELU = auto()
    RELU6 = auto()      # For MobileNet
    GELU = auto()
    SWISH = auto()      # x * sigmoid(x), for EfficientNet
    SIGMOID = auto()
    TANH = auto()
    SOFTMAX = auto()
    ...
    BATCHNORM = auto()
    LAYERNORM = auto()
    GROUPNORM = auto()  # For newer architectures
